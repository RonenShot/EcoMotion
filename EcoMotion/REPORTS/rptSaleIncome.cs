...
{
    public partial class rptSaleIncome : Form
    {
        public rptSaleIncome()
        {
            InitializeComponent();
        }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString("דוח פירוט הכנסות  ", new Font("Tahoma", 14,
        ...
        private void rptSaleIncome_Load(object sender, EventArgs e)
        {
        ...
        public void PresntSql(int r)
        {
            ...
            {
                case 1:
                    stSql = "SELECT Sum(totalCost) As income FROM Sale WHERE Month(Sale.dateSale)= '" +monthSelector_txt.Text+ "' AND Year(Sale.dateSale)= '"+ yearSelector_txt.Text+"'";// GROUP BY Month([Sale.dateSale]),Year([Sale.dateSale]) HAVING (Year([Sale.dateSale])='" + yearSelector_txt.Text+"')"; // סדר עולה
                   
                    break;
                case 2:
                    stSql = "SELECT * FROM Customer ORDER BY ID ASC"; 
                    break;
                case 3:
                    stSql = "select * from Customer where Active='כן'";
                    break;
                case 4:
                    stSql = "select * from Customer where Active='לא'";
                    break;

            }
            dbEcoMotion.GetDataSet((stSql));
            Tsale = dbEcoMotion.ds.Tables[0];
            dataGridView1.DataSource = Tsale;
