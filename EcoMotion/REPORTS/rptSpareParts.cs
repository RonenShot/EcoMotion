...
{
    public partial class rptSpareParts : Form
    {
        public rptSpareParts()
        {
            InitializeComponent();
        }
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString("דוח פירוט חלקי חילוף ", new Font("Tahoma", 14,
        ...
        private void rptSparePartsList_Load(object sender, EventArgs e)
        {
        ...
        public void PresntSql(int r)
        {
            ...
            {
                case 1:
                    stSql = "SELECT * FROM SpareParts ORDER BY idPart ASC"; // סדר עולה
                    break;
                case 2:
                    stSql = "SELECT * FROM SpareParts ORDER BY idPart DESC";//  סדר יורד 
                    break;
                case 3:
                    stSql = "select * from SpareParts where Active='כן'";
                    break;
                case 4:
                    stSql = "select * from SpareParts where Active='לא'";
                    break;
                case 5:
                    stSql = "SELECT * FROM SpareParts WHERE CInt(quantityInStock)<20";
                    break;

            }
            dbEcoMotion.GetDataSet((stSql));
            TspareParts = dbEcoMotion.ds.Tables[0];
            if(TspareParts.Rows.Count!=0)
                 dataGridView1.DataSource = TspareParts;
